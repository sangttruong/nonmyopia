...
class MixtureKernel(Kernel):
        ...
        **kwargs
        ...
        if integer_dims is not None:
            ...
            if not np.all(np.in1d(integer_dims_np, cont_dims_np)):
                raise ValueError(
                    "if supplied, all continuous dimensions with integer constraint must be themselves "
        ...
        self.register_parameter(
            name="raw_lamda", parameter=torch.nn.Parameter(torch.ones(1))
        )
        ...
        if categorical_kern_type == "overlap":
            self.categorical_kern = CategoricalOverlap(
            ...
            self.categorical_kern = TransformedCategorical(
                lengthscale_constraint=categorical_lengthscale_constraint,
                ard_num_dims=len(categorical_dims) if categorical_ard else None,
            )
        else:
            raise NotImplementedError(
                "categorical kernel type %s is not implemented. "
                % categorical_kern_type
            )

        # By default, we use the Matern 5/2 kernel
        if continuous_kern_type == "mat52":
            self.continuous_kern = WrappedMatern(
                nu=2.5,
            ...
            self.continuous_kern = WrappedRBF(
                ard_num_dims=len(continuous_dims) if continuous_ard else None,
                integer_dims=integer_dims,
                lengthscale_constraint=continuous_lengthscale_constraint,
            )
            ...
            raise NotImplementedError(
                "continuous kernel type %s is not implemented. " % continuous_kern_type
            )
    ...
    def forward(self, x1, x2, diag=False, x1_cont=None, x2_cont=None, **params):
        """
        ...
        if x1_cont is None and x2_cont is None:
            assert x1.shape[1] == len(self.categorical_dims) + len(
            ...
            x1_cat, x2_cat = (
                x1[:, self.categorical_dims].detach(),
                x2[:, self.categorical_dims].detach(),
            )
        else:
...
class TransformedCategorical(CategoricalOverlap):
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, exp="rbf", **params):
        ...
        if diag:
            return torch.diag(k_cat).to(dtype=torch.float64)
...
class OrdinalKernel(Kernel):
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        ...
        dist = 1.0 - torch.abs(diff)
        if self.ard_num_dims is not None and self.ard_num_dims > 1:
            k_cat = torch.exp(
                torch.sum(dist * self.lengthscale, dim=-1) / torch.sum(self.lengthscale)
            )
        else:
            k_cat = torch.exp(self.lengthscale * torch.sum(dist, dim=-1) / x1.shape[1])
        if diag:
            return torch.diag(k_cat).float()
        return k_cat.float()
...
class WarpedMaternKernel(MaternKernel):
    def __init__(
        self, input_warping, embedding_bounds: Tensor, nu: float = 2.5, **kwargs
    ):
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        assert self.embedding_bounds.shape == (2, x1.shape[1]), (
            self.embedding_bounds.shape,
            x1.shape,
        )
