...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(len(ENVS), len(KERNELS) + 1, figsize=(4 * (len(KERNELS) + 1), 4 * len(ENVS)))
    for ei, env_name in enumerate(ENVS):
        ...
        axs[ei][0].set(xlabel="$x_1$", ylabel="$x_2$", xlim=bounds_plot_x, ylim=bounds_plot_y)
