...
if args.setting == "default":
    ...
    from ablation_configs.init_samples_nightlight import (
        algos,
        algos_name,
        cost_functions,
        env_discretizeds,
...
def read_results(seed, en, cost_fn, args):
    ...
    for env_name in env_names:
        for algo in algos:
            ...
            print(
                f"Reading {env_name} - {args.kernel} - {algo} - {n_initial_points} - {args.hidden_dim} - {args.n_restarts}"
            )
            base_path = (
            ...
            buffer_file = f"{base_path}/buffer.pt"
...
def draw_result(results, seed, en, cost_fn, args):
    ...
    plt.savefig(
        f"plots/{args.setting}_{args.kernel + '_' if args.kernel != 'RBF' else ''}{en}_{cost_fn}_init{args.n_initial_points}_hidden{args.hidden_dim}_rs{args.n_restarts}.pdf"
    )
    plt.savefig(
        f"plots/{args.setting}_{args.kernel + '_' if args.kernel != 'RBF' else ''}{en}_{cost_fn}_init{args.n_initial_points}_hidden{args.hidden_dim}_rs{args.n_restarts}.png",
        dpi=300,
    )
    plt.close()
