...
def run_script():
    ...
    # --- app-specific below
    config.app = 'multilevelset'
    config.algo = args.algo
    config.fname = 'alpine'
    config.support_num_per_dim = 15
    config.levelset_thresholds = [0.35, 0.6]
    config.num_dim_action = len(config.levelset_thresholds)
...
def plot_levelset_alpine(config, next_x, data, action_samples, iteration):
    ...
    # Save plot and close
    neatplot.save_figure(f'multilevelset_{iteration}', 'pdf', config.save_dir)
    plt.close()
...
def eval_levelset(qhes, config, data, next_x, iteration, random=False):
    ...
    with torch.no_grad():
        ...
        test_samples, all_test_targets = get_test_samples_targets(func, config, random)
        test_pm = postmean(test_samples.unsqueeze(1))  # batch_size x 1 x data_dim
        test_pm = test_pm.detach().numpy()

        all_acc = []
        for i, threshold in enumerate(config.levelset_thresholds):
            test_preds = []
            for val in test_pm:
                test_preds.append(1.0 if val > threshold else 0.0)
            test_preds = np.array(test_preds)
            acc = 1 - np.abs(all_test_targets[i] - test_preds).mean()
            acc = acc.tolist()
            all_acc.append(acc)

        # Return eval_metric and eval_data (or None)
        return np.mean(all_acc), None
...
def get_test_samples_targets(func, config, random=False):
    ...
    for threshold in config.levelset_thresholds:
        ...
        for val in test_samples_values:
            test_targets.append(1.0 if val > threshold else 0.0)
        test_targets = np.array(test_targets)
        all_test_targets.append(test_targets)
    return test_samples, all_test_targets
...
def final_eval_levelset(eval_list, config):
    ...
    neatplot.save_figure('multilevelset_final', 'png', config.save_dir)
    plt.close()
...
if __name__ == "__main__":
    ...
    # fig, ax = plt.subplots(figsize=(6, 6))
    # plot_function_contour(ax, [0.35 / alpine.y_scale, 0.6 / alpine.y_scale])
    # plt.show()
