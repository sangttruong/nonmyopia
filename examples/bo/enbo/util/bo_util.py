    ...
    for iteration in range(1, num_bo_iters + 1, batch_size):
        ...
        try:
            ...
            if optimize_on_grid:
                ...
                objective_func_range = objective_func.sf_bounds[1]-objective_func.sf_bounds[0]
                bounds = torch.tensor([
                    [
                        (new_x[i] - r - objective_func.sf_bounds[0])/objective_func_range,
                        (new_x[i] + r - objective_func.sf_bounds[0])/objective_func_range
                    ] for i in range(objective_func.dim)
