...
class SynthFuncMST(BenchmarkFunction):
    def __init__(
        self, dim=2, noise_std=None, maximize=False
    ):
        super().__init__(noise_std=noise_std, maximize=maximize)

        hypers = {"ls": 1.0, "alpha": 2.0, "sigma": 1e-2, "n_dimx": dim}
        self.sf_bounds = [0, 10]
        sf_domain = [self.sf_bounds] * dim
        self.sf = SynthFunc(seed=12, hypers=hypers, n_obs=50, domain=sf_domain)

        self.dim = dim
        self.bounds = Tensor([self.sf_bounds] * dim).t()
        self._optimal_value = 0.0 ############## TODO ##############
        self.x_star = [[0.0] * dim] ############## TODO ##############

    def evaluate_true(self, x):
        return torch.tensor(self.sf(x))
