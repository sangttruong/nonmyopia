...
for emb_name in emb:
    ...
    plt.fill_between(sizes, np.array(r2s_emb[emb_name]) - np.array(r2s_std_emb[emb_name]), 
                     np.array(r2s_emb[emb_name]) + np.array(r2s_std_emb[emb_name]), alpha=0.1)
plt.ylim(top=1)
plt.ylabel(r"$R^2$")
plt.xlabel("Number of training data")
plt.legend()
plt.savefig("R2_convergence.png", dpi=500)
plt.close()

plt.figure()
for emb_name in emb:
    plt.plot(sizes, rmse_emb[emb_name], label=emb_name)
    plt.fill_between(sizes, np.array(rmse_emb[emb_name]) - np.array(rmse_std_emb[emb_name]), 
                     np.array(rmse_emb[emb_name]) + np.array(rmse_std_emb[emb_name]), alpha=0.1)
plt.ylim(bottom=0)
plt.ylabel(r"$RMSE$")
plt.xlabel("Number of training data")
plt.legend()
plt.savefig("RMSE_convergence.png", dpi=500)
plt.close()
