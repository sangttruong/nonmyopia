...
def _configure_quantization(
    ...
    if getattr(config, "quantization_config", None):  # ptq
        ...
        if quant_method == QuantizationMethod.GPTQ:
            require_version("auto_gptq>=0.5.0",
        ...
        if quant_method == QuantizationMethod.AQLM:
            require_version("transformers>=4.39.0",
        ...
        logger.info(
            "Loading {}-bit {}-quantized model.".format(quant_bits, quant_method.upper()))
        ...
        if is_deepspeed_zero3_enabled() or model_args.quantization_device_map == "auto":
            ...
            require_version("transformers>=4.39.0",
                            "To fix: pip install transformers>=4.39.0")
            require_version("accelerate>=0.28.0",
                            "To fix: pip install accelerate>=0.28.0")
            require_version("bitsandbytes>=0.43.0",
...
def _resize_embedding_layer(model: "PreTrainedModel", tokenizer: "PreTrainedTokenizer") -> None:
    ...
    if len(tokenizer) > current_embedding_size:
        ...
        logger.info("Resized token embeddings from {} to {}.".format(
...
def patch_config(
    ...
    if model_args.moe_aux_loss_coef is not None:
        if getattr(config, "model_type", None) in ["mixtral", "qwen2_moe"]:
            setattr(config, "router_aux_loss_coef",
                    model_args.moe_aux_loss_coef)
    ...
    if not is_deepspeed_zero3_enabled():
        ...
        if init_kwargs["low_cpu_mem_usage"]:
            ...
            elif "device_map" not in init_kwargs:
                init_kwargs["device_map"] = "auto"
...
def patch_valuehead_model(model: "AutoModelForCausalLMWithValueHead") -> None:
    ...
    setattr(model, "get_input_embeddings",
            MethodType(get_input_embeddings, model))
    setattr(model, "create_or_update_model_card",
