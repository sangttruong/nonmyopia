    ...
    for i in range(len(examples[dataset_attr.prompt])):
        ...
        if dataset_attr.history and isinstance(examples[dataset_attr.history][i], list):
            for old_prompt, old_response in examples[dataset_attr.history][i]:
                ...
                prompt.append({"role": Role.ASSISTANT.value,
        ...
        if dataset_attr.response and isinstance(examples[dataset_attr.response][i], list):
            ...
            response = [{"role": Role.ASSISTANT.value,
...
def convert_sharegpt(examples: Dict[str, List[Any]], dataset_attr: "DatasetAttr") -> Dict[str, List[Any]]:
    ...
    for i, messages in enumerate(examples[dataset_attr.messages]):
        ...
        messages = messages[: len(messages) // 2 * 2]
        if len(messages) == 0:
            continue
...
def convert_oracle(examples: Dict[str, List[Any]], dataset_attr: "DatasetAttr") -> Dict[str, List[Any]]:
    outputs = {"text": [], "reward": []}
...
def align_dataset(
    ...
    elif dataset_attr.formatting == "oracle":
