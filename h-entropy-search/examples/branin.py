...
def branin_normalized(x):
    """Branin over a [(0, 1), (0, 1)] unit square, output normalized to be in [-1, 1]."""
...
def transform_to_branin_domain(x_tensor):
    """Transform a tensor in unit square domain to numpy array in branin domain."""
    x_unnorm = copy.deepcopy(x_tensor.detach()).numpy()
    x_unnorm[:, 0] = x_unnorm[:, 0] * (domain[0][1] - domain[0][0]) + domain[0][0]
    x_unnorm[:, 1] = x_unnorm[:, 1] * (domain[1][1] - domain[1][0]) + domain[1][0]

    return x_unnorm


def transform_to_branin_domain_actions(x_tensor):
    """Transform actions tensor in unit square domain to numpy array in branin domain."""
    x_unnorm = copy.deepcopy(x_tensor.detach()).numpy()
    x_unnorm[:, :, 0] = x_unnorm[:, :, 0] * (domain[0][1] - domain[0][0]) + domain[0][0]
    x_unnorm[:, :, 1] = x_unnorm[:, :, 1] * (domain[1][1] - domain[1][0]) + domain[1][0]
...
def transform_to_branin_output(y_tensor):
    """Transform a tensor in [0, 1] to numpy array in branin output range."""
    y_unnorm = copy.deepcopy(y_tensor.detach()).numpy()
    y_unnorm = (-1 * y_unnorm + 1.0) / 2.0
    y_unnorm = y_unnorm * (ylims[1] - ylims[0]) + ylims[0]
