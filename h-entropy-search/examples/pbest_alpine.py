...
def run_script():
    ...
    config = Namespace(
        ...
        func_is_noisy = False,
        func_noise = 0.1,
        plot_iters = list(range(1, 101, 3)),
    )
    # --- app-specific below
    config.app = 'pbest'
    config.fname = 'alpine'
    config.algo = args.algo
    action_domain = [config.bounds_action] * config.num_dim_action
    config.n_rand_samp = 100
    config.rand_samp = uniform_random_sample_domain(action_domain, config.n_rand_samp)
...
def eval_topk(qhes, config, data, next_x, iteration):
    ...
    value_function = qhes.value_function_cls(
        model=qhes.model,
        num_restarts=eval_num_restarts,
        num_fantasies=eval_num_outer_mc,
        rand_samp=config.rand_samp,
    ...
    for i in range(config.acq_opt_iter):
        optimizer.zero_grad()
        #breakpoint()
        losses = -value_function(X_action[:eval_num_restarts, :eval_num_outer_mc, :, :]).mean(dim=0)
        #losses = -value_function(X_action).mean(dim=0)
        loss = losses.sum()
