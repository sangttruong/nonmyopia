...
class qHEntropySearchMinMax(MCAcquisitionFunction):
    def __init__(
        ...
        if sampler is None:
            sampler = SobolQMCNormalSampler(sample_shape=512, collapse_batch_dims=True)
...
class qHEntropySearchLevelSet(MCAcquisitionFunction):
    def __init__(
        ...
        super().__init__(model=model)
        if sampler is None:
            sampler = SobolQMCNormalSampler(sample_shape=128, collapse_batch_dims=True)
        self.sampler = sampler
    ...
    def __init__(
        ...
        if sampler is None:
            ...
            sampler = SobolQMCNormalSampler(
                sample_shape=num_fantasies, resample=False, collapse_batch_dims=True
            )
