...
def generate(save_folder, ackley_function, number_samples, training_ratio):
    ...
    while len(datas) < number_samples:
        sample = torch.tensor(np.random.uniform(-1, 1, DIM)).to(device="cuda", dtype=torch.float64)
...
if __name__ == '__main__':
    ackley_function = test_functions.synthetic.Ackley(dim=DIM, bounds=[(-1.0, 1.0)]*DIM)
    
    generate(save_folder="./data", ackley_function=ackley_function, number_samples=5000, training_ratio=0.7)
