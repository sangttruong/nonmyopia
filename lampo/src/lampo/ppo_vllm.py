...
class LLMActor:
    def __init__(self, model_config):
        ...
        self.vllm_engine = self.vllm_model.llm_engine

    def generate(self, query, sampling_params, **kwargs):
        return self.vllm_model.generate(query, sampling_params, **kwargs)

    def update_weight(self, *args, **kwargs):
        self.vllm_engine.model_executor.driver_worker.update_weight(*args, **kwargs)
...
@ray.remote(num_cpus=HALF_CPUS, num_gpus=HALF_CPUS)
def train_func(args, ppo_config, model_config):
    gpu_ids = ray.get_gpu_ids()
    os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(
        str(int(gpu_id)) for gpu_id in gpu_ids
    )
    ...
    if not model_config.torch_dtype:
    ...
    print("Creating vLLM actor...")
    vllm_actor = LLMActor.options(
        scheduling_strategy=PlacementGroupSchedulingStrategy(placement_group=vllm_pg)
    ).remote(model_config)
    ...
    for step, batch in enumerate(tqdm(ppo_trainer.dataloader)):
        ...
        for rqi in range(num_steps):
            ...
            responses = vllm_actor.generate.remote(
                query, sampling_params=vllm_generation_config
            )
            responses = ray.get(responses)
...
if __name__ == "__main__":
    ...
    # Create a single node Ray cluster with NUM_GPUS GPUs.
    ray.init(num_cpus=NUM_GPUS, num_gpus=NUM_GPUS)

    # Create two placement groups, one for the vLLM actor and one for the PPO trainer.
    vllm_pg = placement_group([{"CPU": 1, "GPU": 1}] * HALF_CPUS)
    ppo_pg = placement_group([{"CPU": 1, "GPU": 1}] * HALF_CPUS)
    ray.get(vllm_pg.ready(), timeout=30)
    ray.get(ppo_pg.ready(), timeout=30)
    print(placement_group_table(vllm_pg))
    print(placement_group_table(ppo_pg))

    # Init PPO trainer
    result = train_func.options(
        scheduling_strategy=PlacementGroupSchedulingStrategy(placement_group=ppo_pg)
    ).remote(args, ppo_config, model_config)
    ray.get(result)
