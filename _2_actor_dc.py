...
from _3_amortized_network_dc import AmortizedNetwork, Project2Range
from _3_amortized_network_antbo import AmortizedNetworkAntBO
from _4_qhes_dc import qMultiStepHEntropySearch
...
class Actor:
    ...
    def query(self, buffer, iteration: int, embedder):
        ...
        prev_X = torch.nn.functional.one_hot(prev_X, num_classes=self.parms.num_categories).to(dtype=self.parms.torch_dtype)
        ...
        if self.parms.algo == "HES":
            ...
            if iteration == self.parms.n_initial_points:
                self.reset_parameters(
                    prev_X=prev_X, prev_y=prev_y, prev_hid_state=prev_hid_state, embedder=embedder
                )
            ...
            next_X = embedder.decode(return_dict["X"][0][idx].reshape(1, -1))
            # next_X = return_dict["X"][0][idx].reshape(1, -1).long()
            next_X = torch.nn.functional.one_hot(next_X, num_classes=self.parms.num_categories).to(dtype=self.parms.torch_dtype)
        ...
        # elif self.parms.algo == "qMSL":
        #     cats = torch.arange(self.parms.num_categories, dtype=torch.long, device=self.parms.device)
        #     x1, x2 = torch.meshgrid(cats, cats)
        #     original_choices = torch.stack([x1.reshape(-1), x2.reshape(-1)], dim=1).reshape(-1, 1, 2)
        #     # n_categories x q x x_dim
            
        #     posteriors = self.acqf.model.posterior(choices)
        #     y = self.acqf.sampler(posteriors)
        #     # n_samples x n_categories x q x 1
            
        #     choices = original_choices[None, ...].expand(y.shape[0], -1, -1)
        #     # n_samples x n_categories x q x x_dim
            
        #     self.acqf.model = self.acqf.model.condition_on_observations(choices, y)
            ...
            if self.parms.env_name == "AntBO":
                ...
                lb = embedder.encode(prev_X[-1]) - self.parms.cost_func_hypers["radius"]
                ub = embedder.encode(prev_X[-1]) + self.parms.cost_func_hypers["radius"]
